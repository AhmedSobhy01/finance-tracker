...
class LendingsController extends Controller
{
    public function index(Request $request)
    {
        $cat = $request->get('cat');
        if ($cat == 'paid'){
            $lendings = Due::lendings()->whereNotNull('paid_at')->latest()->paginate(applicationSettings('pagination_count'));
        }elseif ($cat == 'unpaid'){
            $lendings = Due::lendings()->whereNull('paid_at')->latest()->paginate(applicationSettings('pagination_count'));
        }else{
            $lendings = Due::lendings()->latest()->paginate(applicationSettings('pagination_count'));
        }

        $people = Person::select('id', 'name')->latest()->get();
        return view('lendings.index', compact('lendings', 'people'));
    }
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            if($current_state) {
                ...
                return response_ok(__('messages.title.deleted'), __('messages.body.lending_set_as_unpaid'), $data);
            }else {
                $data = [
                    'id' => $due->id,
                    'paid' => $due->paid_at ? true : false,
                ...
                return response_ok(__('messages.title.deleted'), __('messages.body.lending_set_as_paid'), $data);
            }
        } catch (\Exception $e){
